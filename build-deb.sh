...
set -e
if [ "\$1" = "configure" ]; then
    echo "Configuring passwordless permissions for background updates..."
    printf "%s\n%s\n" "Defaults env_keep += \"DEBIAN_FRONTEND\"" "%sudo ALL=(ALL) NOPASSWD: /usr/bin/apt-get, /usr/bin/snap" > /etc/sudoers.d/bodhi-update-notifier
